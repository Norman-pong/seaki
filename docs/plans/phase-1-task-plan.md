...
| ID | 任务 | 主要产出 | 验收标准 | 依赖 |
| --- | --- | --- | --- | --- |
| M0-00 | 工程骨架与质量门禁 | Rust workspace、前端 workspace、基础 CI 脚本（[CI workflow](../../.github/workflows/ci.yml)）、格式化/测试命令、[开发文档](../development.md)、主平台 sandbox 后端决策记录（[ADR-0001](../decisions/0001-main-platform-sandbox-backend.md)）、前端工具链决策记录（[ADR-0002](../decisions/0002-frontend-toolchain-vite8-oxc.md)） | 空工程能执行格式化、lint/typecheck 和测试占位；CI 使用 Node 22.12+ 与 pnpm，覆盖 `pnpm install --frozen-lockfile`、`pnpm check`、`cargo fmt --check`、`cargo clippy --workspace -- -D warnings`、`cargo test --workspace`；记录第一阶段主平台后端，默认 macOS Seatbelt，若执行环境变化必须更新决策；生成物不进入 Git | 无 |
